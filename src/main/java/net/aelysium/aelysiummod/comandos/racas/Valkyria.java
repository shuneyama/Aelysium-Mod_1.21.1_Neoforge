    ...
    public static int aplicar(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (cfg.status.enabled) {
            for (var a : cfg.status.list) {
                var attrib = Valkyria_Config.getAttribute(a.attribute);
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(p.getName().getString() + " se identificou como Valkyria <3"), true);
        return 1;
    }
}
