    ...
    public static int aplicar(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (cfg.status.enabled) {
            for (var a : cfg.status.list) {
                var attrib = Tiefling_Config.getAttribute(a.attribute);
        ...
        ctx.getSource().sendSuccess(() -> Component.literal(p.getName().getString() + " se identificou como Tiefling <3"), true);
        return 1;
    }
}
